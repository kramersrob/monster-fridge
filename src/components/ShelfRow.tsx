...
export default function ShelfRow({ shelfId, label, monsters, log, userId, onCanClick }: ShelfRowProps) {
  const sortableIds = monsters.map(m => `${shelfId}::${m.id}`);

  function drinkCount(monsterId: string) {
    return log.filter(e => e.monster_id === monsterId && e.user_id === userId).length;
  }
      ...
      <div style={{
        ...
        <SortableContext items={sortableIds} strategy={horizontalListSortingStrategy}>
          <div
            ...
            // hover show drag handle via CSS sibling trick workaround
            onMouseOver={e => {
